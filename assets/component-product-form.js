if ( typeof ProductVariants !== 'function' ) {
	...
	class ProductVariants extends HTMLElement {
        ...
        if(document.querySelector('additional-products')){
          document.querySelector('additional-products').dispatchEvent(new CustomEvent('on:variant:change', {
            bubbles: true,
            detail: {
...
if ( typeof ProductForm !== 'function' ) {
	...
	class ProductForm extends HTMLElement {
      ...
      this.additionalProducts = this.querySelector('additional-products');
		}
		...
		async onSubmitHandler(e) {
			...
			const submitButton = this.querySelector('[type="submit"]');
      let customPropId = null;

			submitButton.classList.add('working');

      if(this.additionalProducts){
        const products = this.additionalProducts.getAdditionalProducts();
        customPropId = products && await this.handleAdditionalProducts(products);
      }

      if(customPropId) {
        const mainGroupInput = this.form.querySelector('input[name="properties[_mainGroupProduct]"]');
        let prodId = mainGroupInput.value;

        mainGroupInput.value = `${mainGroupInput.value}-${customPropId}`;

        setTimeout(() => {
          mainGroupInput.value = prodId;
        }, 1000);
      }
    ...
    async handleAdditionalProducts(products) {
      if(!products) return 

      const items = [];
      const customPropId = new Date().getTime();
      ...
      Object.entries(products).forEach(([key, data]) => {
        items.push({
          ...
          properties: {
            ...
            ['_additionalProduct']: `${data.mainGroupProduct}-${customPropId}`
          }
        })
      })

      const response = await fetch(`${KROWN.settings.routes.cart_add_url}.js`, {
      ...
      console.log('handleAdditionalProducts data', data)
      return customPropId
        // .then((response) => response.json())
